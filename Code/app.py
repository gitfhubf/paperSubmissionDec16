...
GEMINI_API_KEY = "Please provide your Gemini API key here"
...
def run_yolo_inference(model_weights, image_path, conf=0.25):
    """
...
def choose_landing_spot_with_chatgpt(bboxes, img_width, img_height):
    """
    Calls Gemini (gemini-2.5-flash) to determine the best landing spot, returning a bounding box.
    """
    ...
    prompt = f"""
        ...
        3. Otherwise, choose a point farthest from the detected humans or animals and the edge of the image. 
        ...
        - Do not include any markdown, code fences, or additional explanations outside the JSON.
    """
    ...
    try:
        ...
        print("DEBUG reply:", chat_reply)

        # Attempt to extract and parse JSON box
        start = chat_reply.find("{")
        end = chat_reply.rfind("}")
        if start == -1 or end == -1 or end <= start:
            return None

        json_str = chat_reply[start : end + 1]
        chosen_box = json.loads(json_str)
...
def main_drone_landing_flow(model_weights, image_path, interim_path, final_path):
    ...
    5) Otherwise, use Gemini to choose final bounding box and draw it on the *original* image => `final_path`.
